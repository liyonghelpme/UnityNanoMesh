...
namespace MCPForUnity.Editor.Services
{
    ...
    internal sealed class TestRunnerService : ITestRunnerService, ICallbacks, IDisposable
    {
        ...
        private string[] _requestedTestNames;
        ...
        public async Task<TestRunResult> RunTestsAsync(TestMode mode, TestFilterOptions filterOptions = null)
        {
            ...
            try
            {
                ...
                McpLog.Info(
                    $"[TestRunnerService] Execute mode={mode} " +
                    $"filter.testNames={DescribeArray(filter.testNames)} " +
                    $"filter.groupNames={DescribeArray(filter.groupNames)} " +
                    $"filter.categoryNames={DescribeArray(filter.categoryNames)} " +
                    $"filter.assemblyNames={DescribeArray(filter.assemblyNames)}");
                _requestedTestNames = filter.testNames;
        ...
        public void RunStarted(ITestAdaptor testsToRun)
        {
            ...
            try
            {
                ...
                int? matchedRequestedLeafs = CountMatchingLeafTests(testsToRun, _requestedTestNames);
                McpLog.Info(
                    $"[TestRunnerService] RunStarted rootName='{testsToRun?.Name}' rootFullName='{testsToRun?.FullName}' " +
                    $"rootHasChildren={testsToRun?.HasChildren} totalLeafs={total?.ToString() ?? "<unknown>"} " +
                    $"requestedTests={DescribeArray(_requestedTestNames)} matchedRequestedLeafs={matchedRequestedLeafs?.ToString() ?? "<unknown>"} " +
                    $"tree={DescribeTestTree(testsToRun)}");
        ...
        {
            McpLog.Info(
                $"[TestRunnerService] RunFinished rootName='{result?.Name}' rootFullName='{result?.Test?.FullName}' " +
                $"resultState='{result?.ResultState}' passed={result?.PassCount} failed={result?.FailCount} skipped={result?.SkipCount} " +
                $"duration={result?.Duration} leafResults={_leafResults.Count}");
            _requestedTestNames = null;
        ...
        private static int? CountMatchingLeafTests(ITestAdaptor node, string[] requestedNames)
        {
            if (node == null || requestedNames == null || requestedNames.Length == 0)
            {
                return null;
            }

            var requested = requestedNames.Where(n => !string.IsNullOrWhiteSpace(n)).ToArray();
            if (requested.Length == 0)
            {
                return null;
            }

            return CountMatchingLeafTestsRecursive(node, requested);
        }

        private static int CountMatchingLeafTestsRecursive(ITestAdaptor node, string[] requestedNames)
        {
            if (node == null)
            {
                return 0;
            ...
            if (!node.HasChildren)
            {
                var fullName = node.FullName ?? string.Empty;
                var shortName = node.Name ?? string.Empty;
                for (int i = 0; i < requestedNames.Length; i++)
                {
                    var requested = requestedNames[i];
                    if (string.Equals(fullName, requested, StringComparison.Ordinal) ||
                        string.Equals(shortName, requested, StringComparison.Ordinal) ||
                        fullName.EndsWith("." + requested, StringComparison.Ordinal) ||
                        fullName.EndsWith(requested, StringComparison.Ordinal))
                    {
                        return 1;
                    }
            ...
            try
            {
                foreach (var child in node.Children)
                {
                    total += CountMatchingLeafTestsRecursive(child, requestedNames);
                }
            }
            catch
            {
        ...
        private static string DescribeArray(string[] values)
        {
            ...
            var filtered = values.Where(v => !string.IsNullOrWhiteSpace(v)).ToArray();
            return filtered.Length == 0 ? "[]" : $"[{string.Join(", ", filtered)}]";
        }

        private static string DescribeTestTree(ITestAdaptor node)
        {
            if (node == null)
            {
                return "<null>";
            ...
            AppendTestTree(node, entries, 0, 12);
            return string.Join(" | ", entries);
        }

        private static void AppendTestTree(ITestAdaptor node, List<string> entries, int depth, int remainingBudget)
        {
            if (node == null || remainingBudget <= 0)
            {
                return;
            }

            string indent = new string('>', depth);
            entries.Add($"{indent}{node.Name}::{(string.IsNullOrWhiteSpace(node.FullName) ? "<no-full-name>" : node.FullName)}");
            ...
            int childBudget = remainingBudget - 1;
            foreach (var child in node.Children)
            {
                if (childBudget <= 0)
                {
                    entries.Add($"{indent}<truncated>");
                    break;
                }

                AppendTestTree(child, entries, depth + 1, childBudget);
                childBudget--;
            }
        }
