...
- `Doc`：研究笔记和参考资料
- `Native`：原生 NanoMesh baker CLI/库，以及内置的 `meshoptimizer` 源码
- `Packages`：Unity 包依赖
...
![NanoMesh 验证场景截图](Assets/Screenshots/nanomesh-readme-snapshot.png)

## 需求

- Unity 2022.3
- URP
- 可选原生 baker 后端：CMake 3.20+ 和 C++17 编译器

## Mesh 处理

- NanoMesh 可使用 `meshoptimizer` 进行离线 cluster 构建、简化和量化。
- 原生 baker 后端位于 `Native/NanoMeshBaker`，并在 `Native/third_party/meshoptimizer` 内置 `meshoptimizer`。
- 仓库内置参考版本位于 `refProj4/meshoptimizer-master/meshoptimizer-master`。
- 如果 meshoptimizer CLI 后端不可用，烘焙器会回退到 Unity 托管后端。
